...
#[bench_case]
fn bench_read_to_end(b: &mut Bencher) {
    b.iter(|| {
...
fn seek_len() {
    ...
    assert!(c.seek(SeekFrom::End(0)).is_ok());
    let old_pos = c.stream_position().unwrap();
    assert_eq!(c.stream_len().unwrap(), 15);
    assert_eq!(c.stream_position().unwrap(), old_pos);

    assert!(c.seek(SeekFrom::Start(7)).is_ok());
    assert!(c.seek(SeekFrom::Current(2)).is_ok());
    let old_pos = c.stream_position().unwrap();
    assert_eq!(c.stream_len().unwrap(), 15);
    assert_eq!(c.stream_position().unwrap(), old_pos);
}
...
fn seek_position() {
    ...
    assert!(c.seek(SeekFrom::End(-3)).is_ok());
    assert!(c.seek(SeekFrom::Current(1)).is_ok());
    assert!(c.seek(SeekFrom::Current(-5)).is_ok());
    assert_eq!(c.stream_position().unwrap(), 8);
    assert_eq!(c.stream_position().unwrap(), 8);

    assert!(c.rewind().is_ok());
    assert_eq!(c.stream_position().unwrap(), 0);
    assert_eq!(c.stream_position().unwrap(), 0);
}
...
#[test_case]
fn test_read_to_end_capacity() {
    let input = &b"foo"[..];

    // read_to_end() takes care not to over-allocate when a buffer is the
    // exact size needed.
    let mut vec1 = Vec::with_capacity(input.len());
    let mut reader = ExampleSliceReader { slice: input };
    assert!(reader.read_to_end(&mut vec1).is_ok());
...
#[bench_case]
fn bench_take_read(b: &mut Bencher) {
    b.iter(|| {
        let mut buf = [0; 64];
